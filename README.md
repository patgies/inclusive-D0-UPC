...
\frac{d\sigma}{dy\,d^2p_{D^0}} $ for inclusive D⁰ photoproduction in UPCs in the CGC formalism.
...
```
prints the rapidity and differential cross section at that fixed $p_{D^0}$: `y  dsigma_dyd^2pD0` .
...
`FRAG_TYPE` env var picks the c → D⁰ fragmentation function: `BCFY`, `KniehlKramer` (default), or `LHAPDF`. Like `CHANNEL`, it's also set (defaulting to `KniehlKramer`) and exported in `run_local.sh` and `run_many_Pb.sh`, so it can be overridden the same way when using those scripts.
```
