    ...
    const std::string& member_file, int pdg_flavor, double Q)
{
    ...
    if (!in) throw std::runtime_error("MakeHymnDZInterpolator: cannot open " + member_file);
    ...
    while (std::getline(in, line)) {
        ...
        if (!std::getline(in, line))
            throw std::runtime_error("MakeHymnDZInterpolator: truncated Q-grid line in " + member_file);
        std::vector<double> qgrid = ParseDoubles(line);

        if (!std::getline(in, line))
            throw std::runtime_error("MakeHymnDZInterpolator: truncated flavor line in " + member_file);
        ...
        if (flavor_col < 0)
            throw std::runtime_error("MakeHymnDZInterpolator: flavor id not found in grid " + member_file);
        ...
        for (size_t ix = 0; ix < nx; ix++) {
            for (size_t iq = 0; iq < nq; iq++) {
                if (!std::getline(in, line))
                    throw std::runtime_error("MakeHymnDZInterpolator: truncated value grid in " + member_file);
                std::vector<double> row = ParseDoubles(line);
                if ((int)row.size() <= flavor_col)
                    throw std::runtime_error("MakeHymnDZInterpolator: short value row in " + member_file);
    ...
    for (const auto& block : blocks) {
        ...
        for (size_t ix = 0; ix < nx; ix++) {
            ...
            dvals[ix] = xf / zvals[ix];
        }

        return std::unique_ptr<Interpolator>(new Interpolator(zvals, dvals));
    }

    throw std::runtime_error("MakeHymnDZInterpolator: clamped Q not found in any subgrid in " + member_file);
}
