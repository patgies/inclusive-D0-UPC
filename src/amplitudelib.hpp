...
class AmplitudeLib
{
    public:
        ...
        /**
         * Prepare an FT interpolator at fixed x
         */
        void InitializeFTInterpolator(double xbj);
         ...
         */
        double S_k(double kt, double xbj, Representation=FUNDAMENTAL, double pow=1.0);


        FT_Method GetFTMethod() { return ft; }
        void SetFTMethod(FT_Method f) { ft=f; }

    private:
        // [yind][r/kind]
        std::vector< std::vector<double> > n;
        Interpolator *ft_interpolator; // Tabulated FT of dipole amplitude
        std::vector<double> yvals;
        std::vector<double> lnrvals;
        std::vector<double> rvals;
        Interpolator *interpolator;

        bool ft_table;  // If true, FT of dipole amplitude is tabulated

        double interpolator_xbj;  //! xbj at which the interpolator is initialized
        double ft_interpolator_xbj;
        double* tmprarray;
        double* tmpnarray;
